...
//     // Debug doctor slots
//     console.log("📌 Doctor slots for date:", date, doctor.dateSlots[date]);

//           console.log("🕒 Checking slot availability request:");
//           console.log("   Date:", date);
//           console.log("   Start:", slotStartTime, " End:", slotEndTime);
//           console.log("   Doctor slots on this date:", doctor.dateSlots[date]);
...
//     const isDoctorOrAdmin =
//       req.userRole === "admin" ||
//       (session.doctorId && session.doctorId.toString() === userId); // ✅ fixed property
...
//     let slots = [];
//     if (doctor.dateSlots instanceof Map) {
//       slots = doctor.dateSlots.get(date) || [];
//     } else {
//       slots = doctor.dateSlots[date] || [];
//     }
//     // If no slots, fallback to weekly schedule
//     if (!slots.length) {
//       slots = doctor.getAvailabilityForDate(date);
//     }
...
router.post("/", authMiddleware, async (req, res) => {
  try {
    if (req.userRole !== "student") return res.status(403).json({ error: "Access denied: students only" });

    const { doctorId, slotStart, slotEnd, notes, mode } = req.body;
    if (!doctorId || !slotStart || !slotEnd || !mode) {
      return res.status(400).json({ error: "Doctor ID, slotStart, slotEnd, and mode are required" });
    }
    ...
    if (
      doctor.dateSlots instanceof Map
        ? !doctor.dateSlots.has(dateStr)
        : !doctor.dateSlots[dateStr]
    ) {
      // Fallback: generate from weekly schedule
      const fallbackSlots = doctor.getAvailabilityForDate(dateStr).map((s) => ({
        ...s,
        isAvailable: true,
      }));
      if (doctor.dateSlots instanceof Map) {
        doctor.dateSlots.set(dateStr, fallbackSlots);
      } else {
        doctor.dateSlots[dateStr] = fallbackSlots;
      }
      doctor.markModified("dateSlots");
      await doctor.save();
    }
    ...
    console.log("Booking session for date", dateStr, "start", slotStartTime, "end", slotEndTime);

    // Get slots for the requested date
    const slots =
      doctor.dateSlots instanceof Map
        ? doctor.dateSlots.get(dateStr)
        : doctor.dateSlots[dateStr];

    // Check slot availability using robust comparison
    ...
    if (slotIndex !== -1) {
      ...
      if (doctor.dateSlots instanceof Map) {
        doctor.dateSlots.set(dateStr, slots);
      } else {
        doctor.dateSlots[dateStr] = slots;
      }
      doctor.markModified("dateSlots");
      await doctor.save();
    } else {
      return res
        .status(400)
        .json({ error: "Failed to book slot. It may have just been taken." });
    }
...
// The rest of your endpoints for updating status, getting sessions, etc. remain unchanged,
// since these do not impact slot availability logic.

// Include other route handlers here as in your existing file...
